...
if (isset($_SESSION['post']['image_data'])) {
  ...
  $img = date('YmdHis') . '_' . $_SESSION['post'];
}
...
//２．データ取得SQL作成
$stmt = $pdo->prepare("SELECT * FROM kadai_php5_piano");
$status = $stmt->execute();
...
if ($status==false) {
  ...
  $view .= '<tr bgcolor="#b0c4de">
              ...
              <th width="400px">アピールポイント・感想</th>
              <th width="80px">ベストポージング</th>
            </tr>';
  $num = 1;
 ...
 //テーブル中身(while文でデータ最後まで抽出)
    ...
    if ($num % 2) {
      ...
      $view .= '<tr>';
        ...
        // 追加 20230119ここから
        $view .= '<th>';
        $view .= '<img src="images/'.$result['img'].'" alt="noimage" width="160px">' ;
        $view .= '</th>';
        // 追加 20230119ここまで
